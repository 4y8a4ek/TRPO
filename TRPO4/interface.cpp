...
using namespace std;
BaseControler *cont;
controler<PNumber> contP;
controler<INumber> contI;
controler<CNumber> contC;
...
HWND hTrackbar, hEdit, hBase, hHistoryWindow, hEditHistory, hModeComboBox;
...
void UpdateButtonState(HWND hwnd, const int &num)
{
    switch (num)
    {
    case 1:
        EnableWindow(GetDlgItem(hwnd, ID_BUTTONS_START + 10), true); // A
        EnableWindow(GetDlgItem(hwnd, ID_BUTTONS_START + 11), true); // B
        EnableWindow(GetDlgItem(hwnd, ID_BUTTONS_START + 12), true); // C
        EnableWindow(GetDlgItem(hwnd, ID_BUTTONS_START + 13), true); // D
        EnableWindow(GetDlgItem(hwnd, ID_BUTTONS_START + 14), true); // E
        EnableWindow(GetDlgItem(hwnd, ID_BUTTONS_START + 15), true); // F
        EnableWindow(GetDlgItem(hwnd, ID_BUTTON_I), false);
        break;
    case 2:
        EnableWindow(GetDlgItem(hwnd, ID_BUTTONS_START + 10), false); // A
        EnableWindow(GetDlgItem(hwnd, ID_BUTTONS_START + 11), false); // B
        EnableWindow(GetDlgItem(hwnd, ID_BUTTONS_START + 12), false); // C
        EnableWindow(GetDlgItem(hwnd, ID_BUTTONS_START + 13), false); // D
        EnableWindow(GetDlgItem(hwnd, ID_BUTTONS_START + 14), false); // E
        EnableWindow(GetDlgItem(hwnd, ID_BUTTONS_START + 15), false); // F
        EnableWindow(GetDlgItem(hwnd, ID_BUTTON_I), false);           // Кнопка "i"
        break;
    case 3:
        EnableWindow(GetDlgItem(hwnd, ID_BUTTONS_START + 10), false); // A
        EnableWindow(GetDlgItem(hwnd, ID_BUTTONS_START + 11), false); // B
        ...
        EnableWindow(GetDlgItem(hwnd, ID_BUTTONS_START + 14), false); // E
        EnableWindow(GetDlgItem(hwnd, ID_BUTTONS_START + 15), false); // F
        EnableWindow(GetDlgItem(hwnd, ID_BUTTON_I), true);
        break;
    default:
        break;
    }
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
    {
        ...
        SetMenu(hwnd, hMenu);
        hModeComboBox = CreateWindow("COMBOBOX", "", WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST | WS_VSCROLL,
                                     15, 25, 100, 100, hwnd, (HMENU)ID_MODECOMBOBOX, NULL, NULL);
        SendMessage(hModeComboBox, CB_ADDSTRING, 0, (LPARAM) "P-ичные");
        SendMessage(hModeComboBox, CB_ADDSTRING, 0, (LPARAM) "Дробные");
        SendMessage(hModeComboBox, CB_ADDSTRING, 0, (LPARAM) "Комплексные");
        SendMessage(hModeComboBox, CB_SETCURSEL, 0, 0);
        hEdit = CreateWindow("EDIT", "", WS_CHILD | WS_VISIBLE | WS_BORDER | ES_RIGHT,
                             130, 15, 255, 40, hwnd, (HMENU)ID_EDITPOLE, NULL, NULL);
        CreateWindow("STATIC", "Основание:", WS_CHILD | WS_VISIBLE,
                     15, 355, 80, 20, hwnd, NULL, NULL, NULL);
        hBase = CreateWindow("EDIT", "10", WS_CHILD | WS_VISIBLE | WS_BORDER | ES_NUMBER | ES_CENTER,
                             280, 355, 50, 25, hwnd, (HMENU)ID_BASEPOLE, NULL, NULL);
        hTrackbar = CreateWindow(TRACKBAR_CLASS, "", WS_CHILD | WS_VISIBLE | TBS_AUTOTICKS,
                                 100, 355, 175, 25, hwnd, (HMENU)ID_TRACKBAR, NULL, NULL);
        SendMessage(hTrackbar, TBM_SETRANGE, TRUE, MAKELPARAM(2, 16));
        SendMessage(hTrackbar, TBM_SETPOS, TRUE, 10);
        CreateWindow("BUTTON", "i", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
                     290, 290, 40, 40, hwnd, (HMENU)(ID_BUTTON_I), NULL, NULL);
        CreateWindow("BUTTON", "=", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
                     345, 345, 40, 40, hwnd, (HMENU)(207), NULL, NULL);
        CreateWindow("BUTTON", "1/x", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
                     345, 290, 40, 40, hwnd, (HMENU)(206), NULL, NULL);
        CreateWindow("BUTTON", "", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
    ...
    case WM_HSCROLL:
        if ((HWND)lParam == hTrackbar)
        {
            ...
            cout << "Base changed " << stoi(buffer);
        }
        break;
    case WM_COMMAND:
        wmId = LOWORD(wParam);

        if (wmId == ID_MODECOMBOBOX && HIWORD(wParam) == CBN_SELCHANGE)
        {
            int selectedIndex = SendMessage(hModeComboBox, CB_GETCURSEL, 0, 0);

            switch (selectedIndex)
            {
            case 0:
                cont = static_cast<BaseControler *>(&contP);
                UpdateButtonState(hwnd, 1);
                break;
            case 1:
                cont = static_cast<BaseControler *>(&contI);
                UpdateButtonState(hwnd, 2);
                break;
            case 2:
                cont = static_cast<BaseControler *>(&contC);
                UpdateButtonState(hwnd, 3);
                break;
            }
        }
        ...
        if (wmId == 207)
        {
            ...
            char buffer[256];
            GetWindowText(hBase, buffer, sizeof(buffer));
            SetWindowText(hEdit, (cont->execute(stoi(buffer))).c_str());
            cont->add_history(cont->get_operation(), cont->execute(stoi(buffer)));
            cont->clear_number();
            ButtonChange = false;
        }
        ...
        {
            string curnum = cont->get_memory();
            int curbase = cont->get_mem_base();
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    HWND hwnd = CreateWindowEx(0, g_szClassName, "Универсальный калькулятор", WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU,
                               CW_USEDEFAULT, CW_USEDEFAULT, 400, 450, NULL, NULL, hInstance, NULL);
    if (!hwnd)
        return 0;
